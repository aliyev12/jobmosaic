    ...
    <div class="bg-white p-8 rounded-lg shadow-md w-full">
      ...
      @if ($user->avatar)
        <div class="mt-2 flex justify-center">
          <img src="{{ asset('storage/' . $user->avatar) }}" alt="User avatar" class="w-32 h-32 object-cover rounded-full">
        </div>
      @endif
      ...
      <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
        ...
        <button class="w-full bg-green-500 hover:bg-green-600 text-white px-4 py-2 border rounded focus:outline-none"
            ...
            <div class="py-2">
              ...
              <p class="text-gray-800 my-4">
                <a href="{{ asset('storage/' . $applicant->resume_path) }}"
                  class="text-blue-500 text-sm hover:underline flex items-center" download>
              ...
              <form method="POST" action="{{ route('applicant.destroy', $applicant->id) }}"
                ...
                <button type="submit" class="text-red-500 hover:text-red-700 text-sm flex items-center">
                  <i class="mr-2" data-feather="trash"></i> Delete Applicant
                </button>
              </form>
            </div>
          @empty
