...
@if ($mobile)
    <nav x-show="open" @click.away="open = false" id="mobile-menu"
        class="md:hidden  mt-5 pb-4 space-y-2 flex flex-col justify-center items-center">
        ...
        @else
            <x-nav-link url="{{ $signIn['url'] }}" route="login" :mobile="true">{{ $signIn['title'] }}</x-nav-link>
...
@else
    <nav class="hidden md:flex items-center space-x-4">
        ...
        @auth
            ...
            <x-logout-button />
            <x-button-link variant="primary" url="{{ $addJob['url'] }}"
                icon="plus">{{ $addJob['title'] }}</x-button-link>
        ...
        @else
            <x-nav-link url="{{ $signIn['url'] }}" route="login">{{ $signIn['title'] }}</x-nav-link>
